...
class tomato():
    def tomato_setup(self):
        ...
        self.config.enable_stream(rs.stream.depth, self.width_depth, self.height_depth, rs.format.z16, 30)
        self.other_stream, self.other_format = rs.stream.color, rs.format.rgb8
        self.config.enable_stream(self.other_stream, self.width_color, self.height_color, self.other_format, 30)
        ...
        depth = np.zeros((self.width_depth, self.height_depth))
          ...
          with tf.compat.v1.Session(graph=self.detection_graph) as sess:
            def detection_loop(self):
                ...
                for k in range(length):
                    if names_show[k] == 'tomato':
                        ...
                        if not y[k]:
                            ...
                            y[k] = np.diff(y[k])
                            dist = depth_frame.get_distance(int((boxes[k][1] + (boxes[k][3]-boxes[k][1])/2)*self.width_color), int((boxes[k][0] + (boxes[k][2]-boxes[k][0])/2)*self.height_color))
                            _y = list(y[k])
                ...
                for k in range(length):  
                    if names_show[k] == 'tomato':
                        if len(max_depth) > i:
                            cv2.drawMarker(image_np, (int((boxes[k][1] + (boxes[k][3]-boxes[k][1])/2)*self.width_color), int((boxes[k][0] + (boxes[k][2]-boxes[k][0])/2)*self.height_color)), color=(0, 0, 0),markerType=cv2.MARKER_CROSS, thickness=2)
                ...
                cv2.line(image_np,(int(self.width_color/2), 0),(int(self.width_color/2), int(self.height_color)),color=(255, 255,0),thickness=2)
                cv2.line(image_np,(0, int(self.height_color/2)),(int(self.width_color), int(self.height_color/2)),color=(255, 255,0),thickness=2)
                cv2.waitKey(1)
