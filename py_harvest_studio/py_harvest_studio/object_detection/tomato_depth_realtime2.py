...
with detection_graph.as_default():
  with tf.compat.v1.Session(graph=detection_graph) as sess:
    while True:
        ...
        color_image = [color_image[0:int(height_color/2), 0:int(width_color/2)], color_image[0:int(height_color/2), int(width_color/2):width_color],
                       color_image[int(height_color/2):height_color, 0:int(width_color/2)], color_image[int(height_color/2):height_color, int(width_color/2):width_color]]

        image_show = []
        for i in range(4):
          image = Image.fromarray(np.uint8(color_image[i]))

          # the array based representation of the image will be used later in order to prepare the
          # result image with boxes and labels on it.
          image_np = color_image[i]
          ...
          image_show.append(np.asarray(image_np))
        ...
        image_np = concat_tile([[image_show[0], image_show[1]],
                                [image_show[2], image_show[3]]])
        t2 = time.time()
