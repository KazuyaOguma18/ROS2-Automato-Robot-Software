...
with detection_graph.as_default():
  with tf.compat.v1.Session(graph=detection_graph) as sess:
    while True:
        ...
        color_image = imdiv.division(color_image, width_color, height_color, Division_number)

        image_show = [[''for i in range(div)] for j in range(div) ]
        for i in range(div):
            for j in range(div):
        
                image = Image.fromarray(np.uint8(color_image[i][j]))
    
                # the array based representation of the image will be used later in order to prepare the
                # result image with boxes and labels on it.
                image_np = color_image[i][j]
        ...
        image_np = concat_tile(image_show)
        t2 = time.time()
