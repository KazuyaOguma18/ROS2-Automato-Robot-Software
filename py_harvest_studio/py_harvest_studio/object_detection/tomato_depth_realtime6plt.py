    ...
    while True:
        ...
        for k in range(length):
          if names_show[k] == 'tomato':
            #cv2.imshow('depth_of_tomato [%d]'%(k), depth_select[k])
            x[k] ,y[k] ,sum_depth = rs_depth.rs_depth_counter(depth_select[k], k)
        ...
        print(max_depth)
        max_depth_2 = np.sort(max_depth)
        print(max_depth_2)

        max_depth_1 = sorted(range(len(max_depth)), key=max_depth.__getitem__)
        print(max_depth_1)

        for k in range(length):  
            ax = plt.subplot(length, 1, k+1)
            ax.set_xlim([200,400])
            plt.title('tomato_graph [%d]'%(k))
            plt.plot(x[k], y[k])

            cv2.putText(image_np, '%d'%(k), (int((boxes[k][1] + (boxes[k][3]-boxes[k][1])/2)*width_color), int((boxes[k][0] + (boxes[k][2]-boxes[k][0])/2)*height_color)), cv2.FONT_HERSHEY_SIMPLEX, 3.0, (255, 255, 100), thickness=2)
            cv2.drawMarker(image_np, (int((boxes[k][1] + (boxes[k][3]-boxes[k][1])/2)*width_color), int((boxes[k][0] + (boxes[k][2]-boxes[k][0])/2)*height_color)), color=(255, 255, 255),markerType=cv2.MARKER_CROSS, thickness=2)
            '''
            depth_intrinsics = 1.93
            pixel_x = (boxes[k][1] + (boxes[k][3]-boxes[k][1])/2 - 0.5)
            pixel_y = (boxes[k][0] + (boxes[k][2]-boxes[k][0])/2 - 0.5)
            depth = max_depth[k]
            X_meter, Y_meter, Z_meter = rs_dis.convert_depth_pixel_to_metric_coordinate(depth, pixel_x, pixel_y, boxes[k][1]-0.5, boxes[k][3]-0.5 ,depth_intrinsics)
            print(X_meter,Y_meter,Z_meter)
            '''
