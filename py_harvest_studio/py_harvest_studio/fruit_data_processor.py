...
class FruitDataProcessor(Node):
    ...
    def check_duplicate_fruits(self, msg):
        for i in range(len(self.x)):
            for j in range(len(msg.x)):
                ...
                if math.sqrt((self.x[i] - msg.x[j])**2 + (self.x[i] - msg.y[j])**2 + (self.z[i] - msg.z[j])**2) > self.radius[i]:
                    self.x.append(msg.x[j])
                    self.y.append(msg.y[j])
                    self.z.append(msg.z[j])
                    self.radius.append(msg.radius[j])
    ...
    def fruit_position_linear_tf(self):
        ...
        l = []
        for i in range(len(self.x)):
            l.append([abs(c*self.z[i] - a*self.y[i])/math.sqrt(a^2+c^2), i])

        l = sorted(l)
        sorted_x = range(len(self.x))
        sorted_y = range(len(self.x))
        sorted_z = range(len(self.x))
        sorted_radius = range(len(self.x))
        sorted_detect_number = range(len(self.x))
